  ...
  <table>
    <tbody>
      {#if spec.fromPrice}
        ...
        {#if r.fees && spec.feesFinanced}<tr><td>{t('planFees')}</td><td>{fmt.eur(r.fees)}</td></tr>{/if}
        ...
        <tr class="sum"><td>{t('planTotal')}</td><td>{fmt.eur(inp.price + r.notary + r.works + (spec.feesFinanced ? r.fees : 0) + r.guarantee)}</td></tr>
        <tr class="minus"><td>{t('planDown')}</td><td>− {fmt.eur(inp.downPayment)}</td></tr>
      {:else}
        <tr><td>{t('planAmount')}</td><td>{fmt.eur(inp.amount)}</td></tr>
      {/if}
      {#if r.fees && !spec.feesFinanced}<tr class="minus"><td>{t('planFeesUpfront')}</td><td>{fmt.eur(r.fees)}</td></tr>{/if}
