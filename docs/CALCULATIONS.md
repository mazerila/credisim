...
## Rental investment, advanced mode (`src/lib/engine/rental.ts`, rules in `src/lib/data/fr/rental-tax.json`)
Computed year by year over the holding period, for the four French regimes at once.
- **Rent** grows at the rent growth rate; furnished regimes add the furnished premium. **Costs** (charges, property tax, landlord insurance, user's recurring costs) grow with inflation; management is a share of the rent collected. One-off costs are paid in their year; in year 0 they are paid in cash and count as money put in.
- **Micro-foncier:** tax = 70 % × rent × (TMI + 17.2 %); not allowed above €15,000 of rent. **Micro-BIC:** 50 % × rent × (TMI + 18.6 %); ceiling €77,700.
- **Real regime (unfurnished):** interest and borrower insurance first; a deficit from other costs lowers other income up to €10,700 (saving = deficit × TMI), the rest is carried forward 10 years against rental income only.
- **LMNP real regime:** result = rent − costs − interest − insurance − purchase costs (year 1). Losses carry forward 10 years. Depreciation (building = price × (1 − land share) / 30, furniture / 7) cannot create a loss; the unused part carries forward without limit.
- **Resale:** value = price × (1 + growth)^years, minus selling costs. Gain = net sale price − price × (1 + 7.5 % + 15 % if held over 5 years) + building depreciation used (LMNP real, since 2025). Tax = gain × (1 − allowance_IR) × 19 % + gain × (1 − allowance_PS) × 17.2 %, allowances 6 %/year from year 6 (exempt after 22 years) and 1.65 %/1.60 %/9 % (exempt after 30 years). The remaining loan is repaid from the sale.
- **Money tied up:** flows = −(down payment + furniture + year-0 costs), then each year's after-tax cash flow, plus the net sale in the last year. **Return on your money** is the IRR of these flows. **vs. placement** = Σ flow_t × (1 + r)^(years − t): what you have at the end compared with putting the same money (including every top-up) in a placement at r. Positive: the property does better.
- Not modelled: surtax on gains above €50,000, early-repayment penalty, CFE, energy-works deficit bonus, the 3-year letting obligation after a deficit.
